...
function createEdit(x, y, w, h, text, style, parent)
	...
	elseif style == "rread" then
		style = wx.wxTE_RIGHT+wx.wxTE_READONLY
...
function isKeyPressed(key)
	...
	if not tonumber(key) then
		...
		--if key:len() == 1 and (string.char(key) ~= nil) then key = string.char(key) end
	end
...
function fromHEXToRGB(color)
    ...
    if tostring(color):len() == 8 then
    	return tonumber(color:sub(3, 4), 16), 
    		   tonumber(color:sub(5, 6), 16), 
    		   tonumber(color:sub(7, 8), 16), tonumber(color:sub(1, 2), 16)
